  ...
  const fetchHealth = useCallback(async () => {
    try {
      ...
      const avgSuccessRate =
        data.length > 0
          ? data.reduce((acc, curr) => acc + curr.success_rate, 0) / data.length
          : 0;
      const activeAgents = data.filter((metric) => metric.status === "active").length;
  ...
  return (
    <div className="fleet-page">
      <header className="fleet-header">
        <h1 className="title-glow">Fleet Status</h1>
      ...
      <section className="fleet-details">
        ...
        <FleetHealth onSelect={setActivePath} activePath={activePath} />
      </section>
